...
data class UserInfo constructor(
    ...
    var pluginPackages: MutableSet<String> = mutableSetOf(),
    ...
    // -- Study Tracker (Competitive Exam) ----------------------------------
    var studyTargetExam: String = "",                // e.g. "JEE", "UPSC", "NEET"
    var studyTargetDate: String = "",                // exam date yyyy-MM-dd
    var studySubjects: MutableList<String> = mutableListOf(), // subject names
    var studyClasses: MutableList<StudyClass> = mutableListOf(), // scheduled classes
    var studyDailyLogHours: MutableMap<String, Float> = mutableMapOf(), // date -> hours studied
    var studySubjectHours: MutableMap<String, Float> = mutableMapOf(), // subject -> total hours
    var studyMockScores: MutableList<MockTestScore> = mutableListOf(),
    var studyStreak: Int = 0,
    var studyLastLogDate: String = "",               // yyyy-MM-dd of last logged session
    var studyRevisionDates: MutableList<String> = mutableListOf(), // dates user wants reminder

    // -- Plugin Store Economy (spec §3/§4) ----------------------------------
    var pluginOwnership: MutableMap<String, PluginOwnershipState> = mutableMapOf(), // packageName -> state
    var dailyCoinsEarnedDate: String = "",   // yyyy-MM-dd for the 150 GC/day earn cap
    var dailyCoinsEarnedAmount: Int = 0,
    // -- Boss Battle System ------------------------------------------------
    ...
    // -- Game Booster Plugin -----------------------------------------------
    var gameBoosterPackage: String = "",
    var gameBoosterName: String = "",
    var gameApps: MutableSet<String> = mutableSetOf(),

    // -- Bank & Loan System ------------------------------------------------
    var loanAmount: Int = 0,
    var loanRepaidAmount: Int = 0,
    var loanTakenDate: String = "",
    var loanDueDate: String = "",
    var loanInterestRate: Float = 0.05f,
    var loanPenaltyDays: Int = 0,
    var loanXpPenaltyTotal: Int = 0,       // total XP deducted for overdue loan
    var loanStoreLocked: Boolean = false,  // true when overdue > 7 days

    // EMI Plugins — user opts into higher upkeep for XP boost
    var emiEnabledPlugins: MutableSet<String> = mutableSetOf(),

    // Paper trading portfolio
    var stockHoldings: MutableMap<String, StockHolding> = mutableMapOf(),
    var tradingTransactions: MutableList<TradeTransaction> = mutableListOf(),

    // -- Wallpaper Bridge --------------------------------------------------
    var useDeviceWallpaper: Boolean = false,
    var soloLevelingMode: Boolean = false,        // render device wallpaper behind app UI
...
/**
 * Per-user ownership/lifecycle state for a single Plugin Store item.
 * Keyed by packageName in UserInfo.pluginOwnership.
 */
@Serializable
data class PluginOwnershipState(
    var isUnlocked: Boolean = false,
    var status: PluginLifecycleStatus = PluginLifecycleStatus.MISSING,
    var graceDaysRemaining: Int = 0,
    var lastUpkeepPaidWeek: String = "",       // ISO week e.g. "2026-W35"
    var cachedApkPath: String? = null,
    var cachedApkTimestampMillis: Long? = null
)

@Serializable
enum class PluginLifecycleStatus {
    INSTALLED, MISSING, GRACE_PERIOD, FROZEN
}

@Serializable
enum class PluginTier(
    val unlockCost: Int,
    val weeklyUpkeep: Int,
    val xpBonusPercent: Int,
    val gracePeriodDays: Int
) {
    MICRO_UTILITY(unlockCost = 1,   weeklyUpkeep = 1,   xpBonusPercent = 2,  gracePeriodDays = 3),
    LIGHT_PLUGIN(unlockCost = 10,  weeklyUpkeep = 3,   xpBonusPercent = 5,  gracePeriodDays = 3),
    TIER_1_STARTER(unlockCost = 100, weeklyUpkeep = 20,  xpBonusPercent = 10, gracePeriodDays = 3),
    TIER_2_UTILITY(unlockCost = 250, weeklyUpkeep = 50,  xpBonusPercent = 15, gracePeriodDays = 3),
    TIER_3_POWER_USER(unlockCost = 500, weeklyUpkeep = 100, xpBonusPercent = 25, gracePeriodDays = 2)
}

const val PLUGIN_STORE_DAILY_COIN_CAP = 150
const val PLUGIN_STORE_REACTIVATION_PENALTY = 25
...
// -- Bank & Trading Data Classes ------------------------------------------

@kotlinx.serialization.Serializable
data class StockHolding(
    val symbol: String,
    val companyName: String,
    var quantity: Float = 0f,          // fractional shares allowed
    var avgBuyPrice: Float = 0f,       // avg price in INR (= GC)
...
data class TradeTransaction(
    ...
    val quantity: Float,
    val priceAtTrade: Float,           // INR/GC per unit
    val totalGC: Float,
    val date: String
)
