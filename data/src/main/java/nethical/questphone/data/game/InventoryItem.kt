...
enum class InventoryItem(val simpleName: String, val description: String, val icon: Int, val isDirectlyUsableFromInventory : Boolean = false, val availability: Availability = Availability.UNCOMMON, val price: Int = 0, val storeCategory: StoreCategory = StoreCategory.TOOLS) {
    ...
    FULL_FREE_DAY("Full Free Day", description = "Enjoy a full day without any app blocks.", isDirectlyUsableFromInventory = true, icon = R.drawable.quest_analytics, price = 500),
    ...
    VAULT_PASS("Vault Pass", description = "Temporarily disable the Calculator Vault for 1 hour without entering the secret code.", isDirectlyUsableFromInventory = true, icon = R.drawable.quest_analytics, storeCategory = StoreCategory.TOOLS, availability = Availability.UNCOMMON, price = 40),
