...
class QuestRepository @Inject constructor(
    ...
    suspend fun upsertQuest(quest: CommonQuestInfo) {
        // Habit Creation Auto-Clamp (Plugin Store spec §3): rewardAmount = min(inputAmount, 15)
        if (quest.reward > 15) quest.reward = 15
    ...
    suspend fun updateQuest(quest: CommonQuestInfo) {
        questDao.upsertQuest(quest)
    }
    ...
    suspend fun getAllQuestsAsList(): List<CommonQuestInfo> {
        return questDao.getAllQuestsAsList()
    }
}
