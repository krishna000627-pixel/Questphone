...
object AppUpdater {
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                if (remoteCode > currentCode && remoteCode != skipped) {
                    ...
                    // Already up to date — delete any stale cached APK
                    clearCache(ctx)
                    null
                }
    ...
    /** Returns true if the cached APK is for the same or older version than what's installed — stale, should be cleared. */
    fun isCachedStale(ctx: Context): Boolean {
        if (!isCached(ctx)) return false
        return try {
            val pm = ctx.packageManager
            val installedCode = pm.getPackageInfo(ctx.packageName, 0).versionCode
            // Parse versionCode from cached APK
            val apkInfo = pm.getPackageArchiveInfo(apkFile(ctx).absolutePath, 0)
            val cachedCode = apkInfo?.versionCode ?: 0
            cachedCode <= installedCode
        } catch (_: Exception) { false }
    }

    /** Delete cache if the cached APK is already installed or older — call on screen resume. */
    fun clearCacheIfStale(ctx: Context) {
        if (isCachedStale(ctx)) clearCache(ctx)
    }
    ...
    fun clearCache(ctx: Context) {
        try { apkFile(ctx).delete() } catch (_: Exception) {}
    }
