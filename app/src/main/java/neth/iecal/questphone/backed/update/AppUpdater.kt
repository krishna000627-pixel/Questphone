...
object AppUpdater {
    ...
    /**
     * Register a one-shot receiver for MY_PACKAGE_REPLACED.
     * Call this from MainActivity.onResume so that when the app restarts
     * after a successful install the APK cache is deleted automatically.
     * Safe to call multiple times — only fires once per install.
     */
    fun clearCacheOnInstallSuccess(ctx: Context) {
        val prefs = ctx.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
        // Store current versionCode so we can detect a version bump on next launch
        val currentCode = try {
            ctx.packageManager.getPackageInfo(ctx.packageName, 0).versionCode
        } catch (_: Exception) { 0 }
        val lastCode = prefs.getInt("last_seen_version", 0)
        if (currentCode > lastCode) {
            // App just updated — clear any leftover cached APK
            clearCache(ctx)
