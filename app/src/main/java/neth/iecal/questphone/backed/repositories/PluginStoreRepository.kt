...
interface PluginStoreRepositoryEntryPoint {
    fun pluginStoreRepository(): PluginStoreRepository
}

/**
 * Plugin Store spec §2/§3/§4 — ownership, weekly upkeep, grace/frozen lifecycle,
 * the on-disk APK staging cache under cacheDir/plugin_apks/, and a disk-backed
 * catalog cache so the launcher's Distraction Blocker Intercept (§2.1) is never
 * skipped by a race — the catalog is available synchronously from the very first
 * app-open after cold start, before any network round trip completes.
 */
@Singleton
class PluginStoreRepository @Inject constructor(
    ...
    companion object {
        ...
        private const val PREFS_NAME = "plugin_store_cache"
        private const val KEY_CATALOG_JSON = "catalog_json"
    }

    private val prefs = context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
    private val catalogJson = kotlinx.serialization.json.Json { ignoreUnknownKeys = true }
    ...
    // ─── Catalog cache ──────────────────────────────────────────────────────
    // In-memory copy backed by SharedPreferences so the Distraction Blocker
    // Intercept can gate a launch immediately at cold start, before any
    // network fetch resolves — closing the "opened before catalog loaded" gap.

    private val _catalog = MutableStateFlow<Map<String, PluginEntry>>(loadCatalogFromDisk())
    val catalog = _catalog.asStateFlow()

    private fun loadCatalogFromDisk(): Map<String, PluginEntry> {
        val raw = prefs.getString(KEY_CATALOG_JSON, null) ?: return emptyMap()
        return try {
            val entries: List<PluginEntry> = catalogJson.decodeFromString(raw)
            entries.associateBy { it.packageName }
    ...
    fun updateCatalog(entries: List<PluginEntry>) {
        ...
        try {
            prefs.edit().putString(KEY_CATALOG_JSON, catalogJson.encodeToString(entries)).apply()
        } catch (_: Exception) { /* disk cache is best-effort; in-memory value still updated */ }
    }
