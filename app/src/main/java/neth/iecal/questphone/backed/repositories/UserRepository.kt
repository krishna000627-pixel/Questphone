...
import nethical.questphone.data.habitica.HabiticaTask
import nethical.questphone.data.habitica.HabiticaPet
import nethical.questphone.data.habitica.BossFight
...
class UserRepository @Inject constructor(
    ...
    // ── Habitica Dailies / Habits ─────────────────────────────────────
    fun getHabiticaTasks(): List<HabiticaTask> = userInfo.habiticaTasks
    fun addHabiticaTask(t: HabiticaTask) { userInfo.habiticaTasks.add(t); saveUserInfo() }
    fun updateHabiticaTask(t: HabiticaTask) {
        val i = userInfo.habiticaTasks.indexOfFirst { it.id == t.id }
        if (i >= 0) { userInfo.habiticaTasks[i] = t; saveUserInfo() }
    }
    fun deleteHabiticaTask(id: String) { userInfo.habiticaTasks.removeAll { it.id == id }; saveUserInfo() }

    fun completeHabiticaTask(id: String) {
        val t = userInfo.habiticaTasks.find { it.id == id } ?: return
        t.completed = true
        t.streak++
        t.lastCompleted = nethical.questphone.core.core.utils.getCurrentDate()
        addCoins(t.coinReward)
        addXp(t.xpReward)
        // Boss damage
        userInfo.bossFight?.let { boss ->
            if (boss.isActive && !boss.isWon && !boss.isLost) {
                boss.bossCurrentHp = maxOf(0, boss.bossCurrentHp - t.coinReward)
                if (boss.bossCurrentHp <= 0) {
                    boss.isWon = true; addCoins(boss.coinRewardOnWin); addXp(boss.xpRewardOnWin)
                }
            }
        }
        saveUserInfo()
    ...
    fun resetDailyCompletions() {
        ...
        userInfo.habiticaTasks.forEach { t ->
            if (t.type == nethical.questphone.data.habitica.HabiticaTaskType.DAILY &&
                t.lastCompleted != today) {
                t.completed = false
                // Boss rage when daily missed
                userInfo.bossFight?.let { boss ->
                    if (boss.isActive && !boss.isWon && !boss.isLost) {
                        boss.bossRage = minOf(boss.bossRageMax, boss.bossRage + 10)
                        if (boss.bossRage >= boss.bossRageMax) {
                            boss.playerHp = maxOf(0, boss.playerHp - boss.bossDamagePerMiss)
                            boss.bossRage = 0
                            if (boss.playerHp <= 0) boss.isLost = true
                        }
                    }
                }
            }
    ...
    // ── Boss Fight ────────────────────────────────────────────────────
    fun startBossFight(boss: BossFight) { userInfo.bossFight = boss.copy(isActive = true); saveUserInfo() }
    fun getBossFight(): BossFight? = userInfo.bossFight
    fun resetBossFight() { userInfo.bossFight = null; saveUserInfo() }

    // ── Diamonds ─────────────────────────────────────────────────────
    fun getDiamonds(): Int = userInfo.diamonds
    fun addDiamonds(n: Int) { userInfo.diamonds += n; saveUserInfo() }
    fun useDiamonds(n: Int): Boolean {
        if (userInfo.diamonds < n) return false
        userInfo.diamonds -= n; saveUserInfo(); return true
    }

    // ── Custom Achievements ──────────────────────────────────────────
    fun getCustomAchievements(): List<CustomAchievement> = userInfo.customAchievements
    fun addCustomAchievement(a: CustomAchievement) { userInfo.customAchievements.add(a); saveUserInfo() }
    fun completeCustomAchievement(id: String) {
        val a = userInfo.customAchievements.find { it.id == id } ?: return
        a.isCompleted = true
        a.completedDate = nethical.questphone.core.core.utils.getCurrentDate()
        addCoins(a.coinReward)
        saveUserInfo()
    }
    fun deleteCustomAchievement(id: String) { userInfo.customAchievements.removeAll { it.id == id }; saveUserInfo() }

    // ── Stat Points ───────────────────────────────────────────────────
    fun getStatPoints(): StatPoints = userInfo.statPoints
    fun allocateStatPoint(slot: Int) {
        ...
        userInfo.statPoints = when (slot) {
            1 -> sp.copy(value1 = sp.value1 + 1)
            2 -> sp.copy(value2 = sp.value2 + 1)
            3 -> sp.copy(value3 = sp.value3 + 1)
            4 -> sp.copy(value4 = sp.value4 + 1)
            else -> sp
        }
        userInfo.statPointsToAllocate--
        saveUserInfo()
    }
    fun renameStatPoint(slot: Int, name: String) {
        val sp = userInfo.statPoints
        userInfo.statPoints = when (slot) {
            1 -> sp.copy(name1 = name)
            2 -> sp.copy(name2 = name)
            3 -> sp.copy(name3 = name)
            4 -> sp.copy(name4 = name)
            else -> sp
        }
        saveUserInfo()
    }
    fun overwriteUserInfo(newInfo: nethical.questphone.data.UserInfo) {
        userInfo.xp = newInfo.xp
        userInfo.coins = maxOf(userInfo.coins, newInfo.coins)
        userInfo.level = newInfo.level
        userInfo.streak = newInfo.streak
        userInfo.habiticaTasks = newInfo.habiticaTasks
        userInfo.trackers = newInfo.trackers
        userInfo.customAchievements = newInfo.customAchievements
        userInfo.statPoints = newInfo.statPoints
        userInfo.bossFight = newInfo.bossFight
        userInfo.diamonds = newInfo.diamonds
        coinsState.value = userInfo.coins
        saveUserInfo()
    }
