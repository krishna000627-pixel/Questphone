...
class UserRepository @Inject constructor(
    ...
    fun isLoanStoreLocked(): Boolean = userInfo.loanStoreLocked

    fun takeLoan(amount: Int) {
        if (userInfo.loanStoreLocked) return
        ...
        userInfo.loanStoreLocked = false
    ...
    fun processDailyLoanPenalty() {
        ...
        val today = java.time.LocalDate.now()
        if (!today.isAfter(due)) return
        val daysOverdue = java.time.temporal.ChronoUnit.DAYS.between(due, today).toInt()
        userInfo.loanPenaltyDays = daysOverdue
        // Compound 5% daily interest
        val penalty = (getLoanRemaining() * userInfo.loanInterestRate).toInt().coerceAtLeast(1)
        userInfo.loanAmount += penalty
        // Day 4+ → 10 XP penalty per day
        if (daysOverdue >= 4) {
            userInfo.xp = (userInfo.xp - 10).coerceAtLeast(0)
            userInfo.loanXpPenaltyTotal += 10
        }
        // Day 7+ → lock Plugin Store purchases and new loans
        userInfo.loanStoreLocked = daysOverdue >= 7
        saveUserInfo()
    }

    // -- EMI Plugins -------------------------------------------------------
    fun isEmiEnabled(packageName: String): Boolean = userInfo.emiEnabledPlugins.contains(packageName)
    fun setEmiEnabled(packageName: String, enabled: Boolean) {
        if (enabled) userInfo.emiEnabledPlugins.add(packageName)
        else userInfo.emiEnabledPlugins.remove(packageName)
        saveUserInfo()
    }
    fun getEmiEnabledPlugins(): Set<String> = userInfo.emiEnabledPlugins
    fun getEffectiveUpkeep(packageName: String, baseUpkeep: Int, emiMultiplier: Float): Int =
        if (isEmiEnabled(packageName)) (baseUpkeep * emiMultiplier).toInt() else baseUpkeep
    fun getEffectiveXpBonus(packageName: String, baseBonus: Int, emiBonus: Int): Int =
        if (isEmiEnabled(packageName)) baseBonus + emiBonus else baseBonus
    ...
    /** Seeds a fake holding without deducting coins — used to auto-populate
     *  the Portfolio tab on first open so it never shows empty. */
    fun seedFakeHolding(symbol: String, companyName: String, quantity: Float, pricePerUnit: Float) {
        if (userInfo.stockHoldings.containsKey(symbol)) return // don't overwrite real holdings
        userInfo.stockHoldings[symbol] = nethical.questphone.data.StockHolding(
            symbol = symbol, companyName = companyName,
            quantity = quantity, avgBuyPrice = pricePerUnit,
            totalInvested = quantity * pricePerUnit
        )
        saveUserInfo()
    }
