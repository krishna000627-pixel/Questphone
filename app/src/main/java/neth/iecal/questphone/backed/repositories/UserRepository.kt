...
class UserRepository @Inject constructor(
    ...
    fun setGameBoosterPackage(pkg: String) {
        // Remove old booster from pluginPackages if switching
        val old = userInfo.gameBoosterPackage
        if (old.isNotEmpty()) userInfo.pluginPackages.remove(old)
        userInfo.gameBoosterPackage = pkg
        // Add new booster to pluginPackages so it's excluded from the real app list
