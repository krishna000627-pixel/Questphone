...
fun Context.isOnline(): Boolean {
    return try {
        ...
        // NET_CAPABILITY_VALIDATED can be false on WiFi with captive portal detection even when
        // internet is fully accessible. Only check for INTERNET capability to fix WiFi sync.
        capabilities.hasCapability(NetworkCapabilities.NET_CAPABILITY_INTERNET)
    } catch (e: Exception) {
        false // safe fallback — no crash
    }
}
