...
fun generateQuestReminder(context: Context, quest: CommonQuestInfo) {
    ...
    CoroutineScope(Dispatchers.IO).launch {
        ...
        // Quest hasn't reached its scheduled start date yet (e.g. a Day 8
        // quest whose weekday happens to also match Day 1) — don't
        // schedule a reminder for it until start_date arrives.
        if (quest.start_date.isNotEmpty() && quest.start_date > getCurrentDate()) {
            Log.d("Reminder", "Quest ${quest.id} not active yet (starts ${quest.start_date})")
            return@launch
        }
