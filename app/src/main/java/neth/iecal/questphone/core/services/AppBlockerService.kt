...
class AppBlockerService : Service() {
    ...
    private val pluginStoreRepository: neth.iecal.questphone.backed.repositories.PluginStoreRepository by lazy {
        dagger.hilt.android.EntryPointAccessors.fromApplication(
            applicationContext,
            neth.iecal.questphone.backed.repositories.PluginStoreRepositoryEntryPoint::class.java
    ...
    private fun jarvisStartOneListen() {
        ...
        val intent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply {
            ...
            putExtra(RecognizerIntent.EXTRA_LANGUAGE, java.util.Locale.US.toLanguageTag())
    ...
    private fun jarvisHandleVoiceResults(matches: List<String>) {
        ...
        for (text in matches) {
            ...
            // Local AI first (only when loaded) — falls back to cloud
            if (LocalLlmEngine.isReady) {
                jarvisScope.launch {
                    jarvisSpeak("On it…")
                    val prefs = neth.iecal.questphone.app.screens.jarvis.JarvisStorage
                        .loadJarvisPrefs(applicationContext)
                    val prompt = LocalLlmEngine.buildPrompt(query, prefs.personality)
                    val sb = StringBuilder()
                    LocalLlmEngine.generate(prompt) { token, done ->
                        sb.append(token)
                        if (done && sb.isNotBlank()) jarvisSpeak(sb.toString())
                    }
                }
            } else {
                jarvisScope.launch {
    ...
    private fun claimMicrophoneForegroundType() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
            try { startForeground(NOTIFICATION_ID, createNotification(), FOREGROUND_SERVICE_TYPE_DATA_SYNC or FOREGROUND_SERVICE_TYPE_MICROPHONE) }
            catch (e: Exception) { Log.e(TAG, "claimMic: ${e.message}") }
        }
    }
    private fun releaseMicrophoneForegroundType() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
            try { stopForeground(STOP_FOREGROUND_DETACH); startForeground(NOTIFICATION_ID, createNotification(), FOREGROUND_SERVICE_TYPE_DATA_SYNC) }
            catch (e: Exception) { try { startForeground(NOTIFICATION_ID, createNotification()) } catch (_: Exception) {} }
        }
    }
    private fun jarvisHandleAction(action: String?) {
        when (action) {
            ACTION_JARVIS_START -> { jarvisContinuousMode = true; jarvisListening = true; claimMicrophoneForegroundType(); jarvisSetupRecognizer(); refreshNotification(); jarvisStartOneListen() }
            ACTION_JARVIS_LISTEN_ONCE -> { jarvisContinuousMode = false; jarvisListening = true; claimMicrophoneForegroundType(); jarvisSetupRecognizer(); refreshNotification(); jarvisStartOneListen() }
            ACTION_JARVIS_STOP -> { jarvisContinuousMode = false; jarvisDestroyRecognizer(); jarvisStopListening(); releaseMicrophoneForegroundType() }
        }
    }
    ...
    private fun detectAndHandleForegroundApp() {
        ...
        // ── Plugin Store spec §2.1: First-Open Distraction Blocker Intercept ──
        // Catches launches this service's polling can see regardless of who triggered them —
        // Play Store's own post-install "Open" button, the system installer's "Open" button,
        // Chrome's download-complete "Open" notification — none of which route through
        // QuestPhone's own click handlers.
        if (detectedForegroundPackage != null && detectedForegroundPackage != packageName) {
            val catalogEntry = pluginStoreRepository.getCatalogEntry(detectedForegroundPackage)
            if (catalogEntry != null && !pluginStoreRepository.isUnlocked(detectedForegroundPackage)) {
                neth.iecal.questphone.app.screens.etc.PluginUnlockBlockerActivity.launch(
                    applicationContext, detectedForegroundPackage
                )
                // no return — plugin not blocked
            }
        }
        ...
        if (detectedForegroundPackage != null && !isLauncherApp(detectedForegroundPackage)) {
            if (neth.iecal.questphone.core.vault.AppVaultManager.isInVault(applicationContext, detectedForegroundPackage!!)) {
                // Vault temporarily disabled by user — let all vault apps through freely
                if (neth.iecal.questphone.core.vault.AppVaultManager.isVaultDisabled(applicationContext)) {
                    return
                }
                if (!isVaultActive) {
                    showHomwScreenOverlay()
    ...
    private fun shouldShowLockScreen(
        ...
        // ── 1000 GC milestone gate ─────────────────────────────────────────
        // Blocker is disabled until user has EVER reached 1000 GC.
        // Once unlocked, stays unlocked forever even if coins drop below 1000.
        ...
        if (!blockerUnlocked) {
            ...
            if (currentCoins >= 1000) {
                // First time hitting 1000 — unlock blocker permanently
                milestonePrefs.edit().putBoolean("blocker_unlocked", true).apply()
            } else {
                return false // Not yet reached 1000 — skip all blocking
            }
        }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val active = allQuests.filter { q ->
                    !q.is_destroyed &&
                    q.integration_id != BaseIntegrationId.OPTIONAL_QUEST && // optional quests never trigger the reminder overlay
