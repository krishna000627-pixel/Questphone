...
class AppBlockerService : Service() {
    ...
    override fun onCreate() {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                ...
                // MICROPHONE type is added dynamically when Jarvis starts (claimMicrophoneForegroundType)
                // so the OS does not treat the mic as permanently held — fixes audio routing conflicts
            } else {
        ...
        jarvisRecognizer = SpeechRecognizer.createSpeechRecognizer(this).apply {
            setRecognitionListener(object : RecognitionListener {
                override fun onResults(results: android.os.Bundle?) {
                    ...
                    // Confidence scores are parallel to matches[]. Drop low-confidence transcripts
                    // (score < 0.4) that are usually noise — a missing array means the engine
                    // does not report scores, so we accept the result unconditionally.
                    val scores = results?.getFloatArray(SpeechRecognizer.CONFIDENCE_SCORES)
                    val topScore = scores?.getOrNull(0) ?: 1f
                    if (topScore >= 0.4f) {
    ...
    private fun jarvisStartOneListen() {
        ...
        val intent = Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).apply {
            ...
            putExtra(RecognizerIntent.EXTRA_MAX_RESULTS, 1)           // top result only — avoids noise alts triggering wake word
            putExtra(RecognizerIntent.EXTRA_CONFIDENCE_SCORES, true)  // enables confidence filtering
            putExtra(RecognizerIntent.EXTRA_LANGUAGE, java.util.Locale.US.toLanguageTag())
            putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_POSSIBLY_COMPLETE_SILENCE_LENGTH_MILLIS, 1000L)
            putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_COMPLETE_SILENCE_LENGTH_MILLIS, 1500L)
            putExtra(RecognizerIntent.EXTRA_SPEECH_INPUT_MINIMUM_LENGTH_MILLIS, 300L)  // ignore sub-300ms blips
    ...
    private fun jarvisHandleVoiceResults(matches: List<String>) {
        ...
        // Only process the top-ranked result. Looping all alternatives caused noise
        // transcripts (low-ranked guesses) to falsely match the wake word.
        ...
        run {    // single-pass block replacing the old for-loop body
    ...
    private fun claimMicrophoneForegroundType() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
            ...
            } catch (e: Exception) { Log.e(TAG, "claimMic failed: ${e.message}") }
        }
    }
    ...
    private fun releaseMicrophoneForegroundType() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
            try {
                ...
                Log.e(TAG, "releaseMic failed: ${e.message}")
    ...
    private fun jarvisHandleAction(action: String?) {
        when (action) {
            ...
            ACTION_JARVIS_STOP -> {
                ...
                jarvisDestroyRecognizer()
                jarvisStopListening()
                releaseMicrophoneForegroundType()
            }
        }
    }
