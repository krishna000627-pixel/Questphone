...
class GemmaRepository @Inject constructor(
    ...
    /**
     * Strips model thinking/planning text that leaks into responses.
     * Gemini 2.5 and some Gemma models output their reasoning before the actual reply.
     * Strategy: find the first line that is clearly a real reply, drop everything before it.
     */
    private fun stripModelThinking(raw: String): String {
        // 1. Remove explicit thinking tags
        var text = raw
            .replace(Regex("<think>.*?</think>", RegexOption.DOT_MATCHES_ALL), "")
            .replace(Regex("<thinking>.*?</thinking>", RegexOption.DOT_MATCHES_ALL), "")
            .trim()

        // 2. If entire response is bullet-point planning, find where actual reply starts
        val lines = text.lines()
        
        // Patterns that indicate a planning/reasoning line (not a real reply)
        val planningPrefixes = listOf(
            "* ", "• ", "**", "- User", "- Kai", "- Current",
            "User:", "Kai is", "Current Balance", "Current State",
            "Input:", "Goal:", "Warmth:", "Witty", "Context:",
            "Observations:", "Keep it", "Acknowledge", "Maybe a",
            "Don't be", "Call out", "Demand", "Persona:",
            "User Profile", "Screen Time", "The user wants",
            "User says", "Note:", "Plan:", "Step ", "First,",
            "Response strategy", "Approach:"
        )

        val firstRealIdx = lines.indexOfFirst { line ->
            val t = line.trim()
            if (t.isBlank()) return@indexOfFirst false
            // A real reply line doesn't start with any planning prefix
            planningPrefixes.none { prefix -> t.startsWith(prefix) } &&
            // And doesn't look like "* Something:" (planning step)
            !Regex("""^\*\s+\w[\w\s]+:""").containsMatchIn(t)
        }

        return if (firstRealIdx > 0 && firstRealIdx < lines.size) {
            lines.drop(firstRealIdx).joinToString("\n").trim()
        } else {
            text
        }
    }
...
- save_memory is always free""".trimIndent()
    ...
    suspend fun quickChat(prompt: String): Result<String> = withContext(Dispatchers.IO) {
        try {
            ...
            val clean = stripModelThinking(raw)
                .replace(Regex("""^[*\-•"'`]+\s*"""), "")
                .replace(Regex("""["'`*]+$"""), "")
    ...
    suspend fun chat(history: List<ChatMessage>, newMessage: String): Result<AiResponse> =
        withContext(Dispatchers.IO) {
            try {
                ...
                val strippedText = stripModelThinking(rawText)
