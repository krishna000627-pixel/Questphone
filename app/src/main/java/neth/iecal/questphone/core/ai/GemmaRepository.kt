...
val KAI_MODELS = listOf(
    "gemma-3-27b-it"      to "Gemma 4 31B (Default)",
    "gemini-2.0-flash"    to "Gemini 2.0 Flash",
    "gemini-1.5-flash"    to "Gemini 1.5 Flash",
    "gemini-1.5-pro"      to "Gemini 1.5 Pro",
    "gemma-3-12b-it"      to "Gemma 3 12B",
    "gemma-3-4b-it"       to "Gemma 3 4B (Fast)"
)
...
class GemmaRepository @Inject constructor(
    ...
    private val BASE_URL = "https://generativelanguage.googleapis.com/v1beta/models/%s:generateContent"
...
You're warm, witty, encouraging, and talk like a real friend (use emojis naturally 😊🔥💪).
...
- save_memory is always free""".trimIndent()
    ...
    suspend fun chat(history: List<ChatMessage>, newMessage: String): Result<AiResponse> =
        withContext(Dispatchers.IO) {
            try {
                ...
                val url = BASE_URL.format(getModel())
                ...
                val actionDone = executeToolCalls(rawText)
    ...
    private suspend fun executeToolCalls(text: String): String? {
        ...
        regex.findAll(text).forEach { match ->
            ...
            } catch (e: Exception) { Log.e("Kai", "Tool error: ${e.message}") }
        }
