...
class GemmaRepository @Inject constructor(
    ...
    // API key stored in KaiPrefs
    fun getApiKey(): String {
        return KaiPrefs.getApiKey(context)
    }
    ...
    private suspend fun buildSystemPrompt(): String {
        ...
        // ── My Life context ──────────────────────────────────────────────────────
        val myLife = MyLifeStorage.load(context)
        val profile = myLife.profile
        val myLifeContext = buildString {
            if (profile.setupComplete) {
                appendLine("\n🕉️ USER'S SPIRITUAL & LIFE PROFILE (My Life):")
                if (profile.name.isNotBlank()) appendLine("  Name: ${profile.name}")
                if (profile.dharma.isNotBlank()) appendLine("  Dharma/Life Purpose: ${profile.dharma}")
                if (profile.fiveYearGoal.isNotBlank()) appendLine("  5-Year Vision: ${profile.fiveYearGoal}")
                if (profile.dominantTrait.isNotBlank()) appendLine("  Dominant Trait: ${profile.dominantTrait}")
                if (profile.spiritualDescription.isNotBlank()) appendLine("  Spiritual Nature: ${profile.spiritualDescription}")
                appendLine("  Lagna: ${KundaliData.lagna}")
                appendLine("  Rashi: ${KundaliData.rashiMoon}")
                appendLine("  Birth Star: ${KundaliData.birthStar}")
                // Current dasha
                val curDasha = KundaliData.dashas.firstOrNull { it.isCurrent }
                if (curDasha != null) appendLine("  Current Mahadasha: ${curDasha.planet} (${curDasha.from}–${curDasha.to}): ${curDasha.note.take(120)}")
                // Active struggles (not overcome)
                val activeStruggles = myLife.struggles.filter { !it.isOvercome }
                if (activeStruggles.isNotEmpty()) {
                    appendLine("  Current Struggles: ${activeStruggles.take(5).joinToString("; ") { it.title }}")
                }
                // Last 3 journal entries summary
                val recentJournal = myLife.journal.take(3)
                if (recentJournal.isNotEmpty()) {
                    appendLine("  Recent Journal: ${recentJournal.joinToString(" | ") { "${it.date}: ${it.content.take(60)}" }}")
                }
                // Mantras
                if (myLife.mantras.isNotEmpty()) {
                    appendLine("  Personal Mantras: ${myLife.mantras.take(3).joinToString(", ") { it.title }}")
                }
            }
        }

        // ── Study Tracker context ─────────────────────────────────────────────
        val today = java.time.LocalDate.now().toString()
        val studyHoursToday = run {
            val classHours = u.studyDailyLogHours[today] ?: 0f
            val screenHours = u.studyDailyLogHours["screen_$today"] ?: 0f
            classHours + screenHours
        }
        val daysToExam = runCatching {
            java.time.temporal.ChronoUnit.DAYS.between(
                java.time.LocalDate.now(), java.time.LocalDate.parse(u.studyTargetDate))
        }.getOrNull()
        ...
        val studyContext = buildString {
            appendLine("\n📚 STUDY TRACKER:")
            if (u.studyTargetExam.isNotBlank()) {
                append("  Target: ${u.studyTargetExam}")
                if (daysToExam != null) append(" — $daysToExam days remaining")
                appendLine()
            }
            appendLine("  Today: ${String.format("%.1f", studyHoursToday)}h / ${u.dailyStudyQuotaHours}h goal | Study streak: ${u.studyStreak} days")
            if (u.studySubjects.isNotEmpty())
                appendLine("  Subjects: ${u.studySubjects.joinToString(", ")}")
            if (todayClasses.isNotEmpty())
                appendLine("  Today's classes: ${todayClasses.joinToString(", ") { "${it.name} (${it.durationHours}h)" }}")
            if (lastMock != null) {
                val pct = lastMock.obtainedMarks * 100 / lastMock.totalMarks.coerceAtLeast(1)
                appendLine("  Last mock: ${lastMock.examName} — $pct% on ${lastMock.date}")
            }
            if (u.studySubjectHours.isNotEmpty()) {
                appendLine("  Subject totals: ${u.studySubjectHours.entries.sortedByDescending { it.value }
                    .take(3).joinToString(", ") { "${it.key}: ${String.format("%.1f", it.value)}h" }}")
            }
        }

        // ── People DB context ─────────────────────────────────────────────────
        val people = PeopleDatabase.load(context)
        val peopleContext = if (people.isNotEmpty()) buildString {
            appendLine("\n👥 PEOPLE DATABASE (${people.size} entries):")
            people.take(10).forEach { p ->
                append("  ${p.emoji} ${p.name}")
                if (p.relation.isNotBlank()) append(" (${p.relation})")
                if (p.notes.isNotBlank()) append(" — ${p.notes.take(60)}")
                appendLine()
            }
            if (people.size > 10) appendLine("  ...+${people.size - 10} more")
        } else ""

        return """You are Kai 🤖, a personal AI companion embedded in QuestPhone - a gamified Android launcher.
$personalityInstructions
CRITICAL: Reply DIRECTLY. NEVER show planning steps, bullet analysis, reasoning chains, or internal thinking. Output ONLY your final response to the user — no preamble, no "User wants:", no "Goal:", no "Warmth:", no bullet breakdowns. Just speak naturally.
...
$installedAppNames$myLifeContext$peopleContext$studyContext
...
- save_memory is always free""".trimIndent()
    ...
    suspend fun quickChat(prompt: String): Result<String> = withContext(Dispatchers.IO) {
        try {
            ...
            // Strip thinking + quotes + bullets, take first real line
            val clean = raw.trim()
                .replace(Regex("""^[*\-•"'`]+\s*"""), "")
                .replace(Regex("""["'`*]+$"""), "")
    ...
    suspend fun chat(history: List<ChatMessage>, newMessage: String): Result<AiResponse> =
        withContext(Dispatchers.IO) {
            try {
                ...
                val body = JSONObject().apply {
                    put("contents", contents)
                    // systemInstruction is the correct way to pass system prompt to Gemma/Gemini
                    // Using fake user/model turns causes the model to mirror that format in replies
                ...
                // Strip all model thinking/planning leakage before displaying
                val strippedText = rawText.trim()
