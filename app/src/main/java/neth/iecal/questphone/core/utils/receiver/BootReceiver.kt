    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (intent?.action == Intent.ACTION_BOOT_COMPLETED || intent?.action == Intent.ACTION_MY_PACKAGE_REPLACED) {
            context?.let {
                ...
                if (neth.iecal.questphone.core.utils.termux.TermuxIntegration.isBridgeEnabled(it)) {
                    neth.iecal.questphone.core.utils.termux.TermuxIntegration.setBridgeEnabled(it, true)
                }
            }
        }
    }
