...
enum class IntegrationId(
    ...
    /**
     * A fully optional quest: same "just mark it done" completion flow as
     * Swift Mark, but never counts toward daily streak/mandatory-quest
     * checks or missed-quest/lockdown-escalation tracking. Reuses Swift
     * Mark's setup and view screens since the completion flow is
     * identical — the only difference is semantic (optional vs required),
     * which callers should check via `integration_id == OPTIONAL_QUEST`
     * wherever daily completion/streak logic decides what's mandatory.
     */
    OPTIONAL_QUEST(
        label = "Optional Quest",
        description = "A bonus quest you can complete for extra coins — skipping it never breaks your streak or triggers a missed-quest reminder.",
        setupScreen = {id,navController ->
            SetSwiftMark(
                id,
                navController
        ...
        rewardCoins = 1,
        docLink = "https://raw.githubusercontent.com/questphone/docs/refs/heads/main/integration/OptionalQuest.md"
    )
}