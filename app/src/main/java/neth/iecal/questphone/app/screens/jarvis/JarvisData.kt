...
data class JarvisPrefs(
    ...
    val offlineAiEnabled: Boolean = false,
    // Base URL of the llama.cpp server, e.g. "http://192.168.1.5:8080"
    // For Termux on same device use "http://localhost:8080"
    val llamaCppUrl: String = "",
    // Model name to pass in the API request — usually the filename without .gguf
    // Leave blank to use whatever the server loaded ("local-model" is sent)
    val llamaCppModel: String = ""
)
...
object JarvisStorage {
    ...
    fun saveJarvisHistory(ctx: Context, history: List<JarvisMessage>) {
        prefs(ctx).edit().putString(KEY_HISTORY, json.encodeToString(history.takeLast(100))).apply()
    }
