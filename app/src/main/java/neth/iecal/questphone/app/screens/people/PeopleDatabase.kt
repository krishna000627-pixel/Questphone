...
/** Three visual styles for the People list screen */
enum class PeopleListStyle { CARD, COMPACT, GRIMOIRE }
...
object PeopleDatabase {
    ...
    private const val KEY_STYLE = "list_style"
    ...
    fun getListStyle(context: Context): PeopleListStyle {
        val name = prefs(context).getString(KEY_STYLE, PeopleListStyle.CARD.name)
        return PeopleListStyle.entries.firstOrNull { it.name == name } ?: PeopleListStyle.CARD
    }

    fun setListStyle(context: Context, style: PeopleListStyle) {
    ...
    val RELATION_OPTIONS = listOf(
        ...
        "Teacher", "Mentor", "Colleague", "Crush", "Partner",
        "Rival", "Acquaintance", "Other"
    )
