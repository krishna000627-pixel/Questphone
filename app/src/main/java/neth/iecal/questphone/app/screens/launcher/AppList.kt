...
fun AppList(navController: NavController, viewModel: AppListViewModel) {
    ...
    if (showHideConfirm.isNotEmpty()) {
        AlertDialog(
            ...
            title = { Text("Hide App") },
            text = { Text("Hide this app from the launcher? It will be treated as a distraction app and require coins to open.") },
            confirmButton = {
            ...
            dismissButton = {
                TextButton(onClick = { viewModel.dismissHideConfirm() }) {
                    Text("Cancel")
                }
            }
        )
