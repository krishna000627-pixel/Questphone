...
fun AppList(navController: NavController, viewModel: AppListViewModel) {
    ...
    // Virtual package names for built-in screens
    val PEOPLE_PKG   = "neth.iecal.questphone.people_db"
    val ROUTINES_PKG = "neth.iecal.questphone.routines"
    val VAULT_PKG    = "neth.iecal.questphone.vault"
    val virtualPackages = setOf(PEOPLE_PKG, ROUTINES_PKG, VAULT_PKG)
                            ...
                            .combinedClickable(
                                onClick = {
                                    when (app.packageName) {
                                        PEOPLE_PKG   -> navController.navigate(RootRoute.PeopleDatabase.route)
                                        ROUTINES_PKG -> navController.navigate(RootRoute.Routines.route)
                                        VAULT_PKG    -> navController.navigate(RootRoute.DataVault.route)
                                        else -> try { viewModel.onAppClick(app.packageName) } catch (_: Exception) {}
                                    }
                                },
                                onLongClick = {
                                    // Virtual entries have no long-press menu
                                    if (app.packageName !in virtualPackages) {
