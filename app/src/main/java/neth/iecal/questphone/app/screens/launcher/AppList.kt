...
fun AppList(navController: NavController, viewModel: AppListViewModel) {
    ...
    // Request focus after first composition - NOT inside onGloballyPositioned
    // (mutating state during layout causes StackOverflow via infinite remeasure loop)
    LaunchedEffect(Unit) {
    ...
    // Reset pull-down guard and reload apps when re-entering this screen
    // This ensures stranger mode whitelist takes effect without needing a restart
    LaunchedEffect(Unit) {
