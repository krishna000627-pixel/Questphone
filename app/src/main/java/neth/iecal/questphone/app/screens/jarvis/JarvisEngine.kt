...
class JarvisEngine(
    ...
    private fun matchBuiltIn(raw: String, tokens: List<String>): JarvisResponse {
        ...
        if (raw.containsAny("study today", "hours today", "how much studied", "study progress")) {
            val today = java.time.LocalDate.now().toString()
            val classH = userInfo.studyDailyLogHours[today] ?: 0f
            val screenH = userInfo.studyDailyLogHours["screen_$today"] ?: 0f
            val total = classH + screenH
            val goal = userInfo.dailyStudyQuotaHours
            val pct = if (goal > 0) (total / goal * 100).toInt() else 0
            return respond("Today: ${String.format("%.1f", total)}h / ${goal}h ($pct%). Study streak: ${userInfo.studyStreak} days.", personality)
        }
        if (raw.containsAny("days to exam", "days till exam", "exam countdown", "exam days left")) {
            val exam = userInfo.studyTargetExam
            val days = runCatching {
                java.time.temporal.ChronoUnit.DAYS.between(
                    java.time.LocalDate.now(), java.time.LocalDate.parse(userInfo.studyTargetDate))
            }.getOrNull()
            return if (days != null) respond("$days days until $exam. Make every day count.", personality)
            else respond("No exam date set. Configure it in Study Tracker settings.", personality,
                JarvisAction.Navigate(RootRoute.StudyTracker.route))
        }
        if (raw.containsAny("mock score", "last mock", "my mock", "mock test")) {
            val mock = userInfo.studyMockScores.firstOrNull()
            return if (mock != null) {
                val pct = mock.obtainedMarks * 100 / mock.totalMarks.coerceAtLeast(1)
                respond("Last mock: ${mock.examName} — $pct% (${mock.obtainedMarks}/${mock.totalMarks}) on ${mock.date}.", personality)
            } else respond("No mock scores logged yet. Add one in Study Tracker.", personality,
                JarvisAction.Navigate(RootRoute.StudyTracker.route))
        }
        if (raw.containsAny("my subjects", "study subjects", "which subjects")) {
            val subjects = userInfo.studySubjects
            return if (subjects.isNotEmpty()) respond("Your subjects: ${subjects.joinToString(", ")}.", personality)
            else respond("No subjects set. Configure in Study Tracker settings.", personality)
        }
        if (raw.containsAny("today classes", "my classes today", "class today", "classes today")) {
            val todayClasses = userInfo.studyClasses.filter {
                java.time.LocalDate.now().dayOfWeek.value in it.daysOfWeek
            }
            return if (todayClasses.isNotEmpty())
                respond("Today's classes: ${todayClasses.joinToString(", ") { "${it.name} at ${it.startTime}" }}.", personality)
            else respond("No classes scheduled for today.", personality)
        }
        if (raw.containsAny("open study tracker", "study tracker")) {
            navCallback(RootRoute.StudyTracker.route)
            return respond("Opening Study Tracker.", personality)
        }
    ...
    private fun evalSimpleMath(expr: String): String {
        ...
        return when {
            ...
            else -> e.trim().toDoubleOrNull()?.toString() ?: throw NumberFormatException("not a number")
        }
    }
