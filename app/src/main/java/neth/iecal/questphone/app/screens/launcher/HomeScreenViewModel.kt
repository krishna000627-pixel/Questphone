...
class HomeScreenViewModel @Inject constructor(
    ...
    val homeQuestCount = MutableStateFlow(homePrefsSp.getInt("quest_count", 3))
    ...
    suspend fun filterQuests() {
        ...
        // Step 5: show quests active right now first, limited by user's count preference
        val currentHour = java.util.Calendar.getInstance().get(java.util.Calendar.HOUR_OF_DAY)
        val activeNow = merged.filter { QuestHelper.isInTimeRange(it) }
        val notActive = merged.filterNot { QuestHelper.isInTimeRange(it) }
        val sorted = (activeNow + notActive)
        questList.value = sorted.take(homeQuestCount.value)
