...
class HomeScreenViewModel @Inject constructor(
    ...
    // -- Quest count on home (1-3, saved as preference) --------------
    private val homePrefsSp =
        application.applicationContext.getSharedPreferences("home_prefs", MODE_PRIVATE)
    val homeQuestCount = MutableStateFlow(homePrefsSp.getInt("quest_count", 1))

    fun setHomeQuestCount(count: Int) {
        homePrefsSp.edit(commit = true) { putInt("quest_count", count) }
        homeQuestCount.value = count
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) { filterQuests() }
    }

    // -- Expose userInfo for cultivation card ------------------------
    ...
    init {
        ...
        // Reactively observe quest DB - updates homescreen instantly when quests change
    ...
    suspend fun filterQuests() {
        ...
        val rawQuests = questRepository.getAllQuestsAsList()
        ...
        val filtered = rawQuests.filter { quest ->
            !quest.is_destroyed &&
                    ...
                    (quest.start_date.isEmpty() || quest.start_date <= todayDate) &&
                    (quest.auto_destruct.isEmpty() || quest.auto_destruct >= todayDate)
        }
        ...
        val uncompleted = filtered.filterNot { it.id in completedIds }.sortedWith(
            compareByDescending<CommonQuestInfo> { QuestHelper.isInTimeRange(it) }
                .thenBy { it.integration_id == BaseIntegrationId.OPTIONAL_QUEST } // required quests before optional
        ...
        // Step 5: Show ONLY quests active right now (time range matches current hour).
        // If none are active right now, fall back to the first uncompleted quest.
        val activeNow = merged.filter { QuestHelper.isInTimeRange(it) && it.id !in completedIds }
        questList.value = if (activeNow.isNotEmpty()) {
            // Show up to homeQuestCount active quests for the current time slot
            activeNow.take(homeQuestCount.value)
        } else {
            // No time-specific quest right now — show first uncompleted
            merged.filterNot { it.id in completedIds }.take(homeQuestCount.value)
        }
