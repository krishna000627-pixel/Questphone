...
# TOKEN is never stored here — read from env or prompt
if [ -z "$GH_TOKEN" ]; then
  echo "Enter your GitHub PAT (hidden):"
  read -rs GH_TOKEN
  echo ""
fi
...
# Block .sh files from being committed — this is what caused the secret leak
